"""
rag_search_fixed.py — CLI-инструмент семантического поиска по RAG-каталогу.

УСТАРЕЛО: используйте rag_search.py (поддерживает --url для Docker Qdrant).
Этот файл оставлен для обратной совместимости.

Использует общий RAGSearcher из rag_core.py.

Режимы:
  Интерактивный:  python rag_search_fixed.py
  CLI:            python rag_search_fixed.py --query "договоры газпром" --limit 5
  Фильтр:         python rag_search_fixed.py --query "счета" --type .xlsx
  Только текст:   python rag_search_fixed.py --query "паспорта" --content-only
  Docker Qdrant:  python rag_search_fixed.py --url http://localhost:6333 --query "..."
"""

# ── WMI-патч (Python 3.14) — до любого импорта torch/transformers ─────────
...
from rag_core import RAGSearcher, load_config
    ...
    while True:
        ...
        try:
            results = searcher.search(
                query, limit=10, file_type=file_type, content_only=content_only
            )
...
def cli_search(
    ...
    try:
        results = searcher.search(query, limit, file_type, content_only)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Примеры:\n"
            "  python rag_search_fixed.py\n"
            "  python rag_search_fixed.py --query \"договоры газпром\" --limit 5\n"
            "  python rag_search_fixed.py --query \"счета\" --type .xlsx\n"
            "  python rag_search_fixed.py --url http://localhost:6333 --query \"паспорта\"\n"
        ),
    )
    parser.add_argument("--url", default=cfg.get("qdrant_url", ""), dest="qdrant_url",
                        help="URL Qdrant-сервера (например http://localhost:6333)")
    parser.add_argument("--db", default=cfg["qdrant_db_path"], help="Путь к локальной базе Qdrant")
    ...
    # Переопределяем конфиг аргументами CLI
    ...
    try:
        ...
        if not searcher.connected:
            logger.error("Не удалось подключиться к Qdrant. Запустите индексирование сначала.")
            return 1
    except Exception as exc:
