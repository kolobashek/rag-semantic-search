...
- DONE 2026-07-11: machine-readable `pilot_gate` объединяет health/recovery/UI-audit/full-pytest/retrieval/sign-off, сверяет source fingerprints и не допускает ложный GO; текущий verdict после `596 passed` — `NO_GO` только по human-labeled retrieval/no-answer/ACL evidence и фактическому customer sign-off/update rehearsal.
- MEASURED 2026-07-11: controlled `release_v2` на той же collection сохранил recall@10 `0.96875` и MRR `1.0`, поднял nDCG `0.9931 -> 0.9998`, снизил p50 `1.924 -> 1.709 с`, но p95 остался вне 3-секундного gate (`3.998 -> 4.012 с`); production preset не переключён.
...
7. NO_GO 2026-07-11: retrieval eval и decision gate реализованы; controlled legacy baseline на 32 запросах дал recall@10 0.96875, MRR 1.0, p50 1.924 с, p95 3.998 с. Создана human review queue из 32 запросов с top-10 candidates, но без автозаполнения ground truth; локальный review UI поддерживает relevant/forbidden/no-answer разметку и атомарное сохранение. Finalize требует reviewer identity/time, минимум 3 no-answer и 3 forbidden cases. Controlled `release_v2` сохранил recall/MRR, улучшил nDCG и p50, но p95 4.012 с не прошёл 3-секундный gate, поэтому production preset не переключён. До GO data owner размечает queue, после чего повторяются legacy/release_v2 и только затем multilingual dense/reranker shadow experiments; нулевой ACL leakage принимается только при `acl_results_checked > 0`.
