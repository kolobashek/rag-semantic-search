...
7. NO_GO 2026-07-11: retrieval eval и decision gate реализованы; controlled legacy baseline на 32 запросах дал recall@10 0.96875, MRR 1.0, p50 1.924 с, p95 3.998 с. Создана human review queue из 32 запросов с top-10 candidates, но без автозаполнения ground truth; локальный review UI поддерживает relevant/forbidden/no-answer разметку и атомарное сохранение. Finalize требует reviewer identity/time, минимум 3 no-answer и 3 forbidden cases. Исправленный настоящий `release_v2` сохранил recall/MRR и прошёл warm latency gate с p50 0.552 с / p95 1.967 с; первый июльский artifact с p95 4.012 с был ошибочно маркирован при фактическом legacy pipeline. Production preset не переключён до human review. После разметки повторяются legacy/release_v2 и только затем multilingual dense/reranker shadow experiments; нулевой ACL leakage принимается только при `acl_results_checked > 0`.
