...
- DONE 2026-07-11: machine-readable `pilot_gate` объединяет health/recovery/UI-audit/full-pytest/retrieval/sign-off, сверяет source fingerprints и не допускает ложный GO; текущий verdict после `603 passed` — `NO_GO` только по human-labeled retrieval/no-answer/ACL evidence и фактическому customer sign-off/update rehearsal.
- SUPERSEDED 2026-07-11: первый июльский artifact с меткой `release_v2` фактически сохранил `retrieval_pipeline=legacy` из resolved config и не является доказательством candidate performance.
- MEASURED 2026-07-11: после исправления CLI preset provenance и metadata/BM25 index настоящий controlled `release_v2` сохранил recall@10 `0.96875` и MRR `1.0`; повторный warm run дал p50 `0.552 с`, p95 `1.967 с`, latency и zero-recall-regression gates пройдены. Production preset остаётся legacy до human ground truth/no-answer/ACL review.
- DONE 2026-07-11: локальный Retrieval ground-truth UI позволяет data owner размечать relevant/forbidden paths, добавлять no-answer cases и сохраняет каждое решение атомарно с `.bak`; browser smoke подтвердил переход по pending queue, сохранение прогресса и отсутствие overflow на 480 px.
- DONE 2026-07-14: retrieval golden доведён до breadth-порога: 50 запросов, 10 no-answer, 31 document-grounded, 11 фактически проверенных chunk-grounded cases и 7 категорий. Eval канонизирует пробелы, чтобы однострочные reviewer excerpts совпадали с payload, содержащим переносы строк.
- DONE 2026-07-11: operations health читает active-WAL `index_state.db` через query-only connection с busy timeout/retry; транзиентное Windows SHM mapping больше не даёт ложный `disk I/O error`, concurrent writer покрыт тестом.
...
7. NO_GO 2026-07-14: retrieval eval и decision gate реализованы; golden достиг breadth-порогов 50/10/31/11/7 для queries/no-answer/document/chunk/categories, а reviewer показывает цитаты кандидатов без автозаполнения ground truth. Последний подтверждённый `release_v2` на старой коллекции сохранил recall/MRR и прошёл warm latency gate с p50 0.552 с / p95 1.967 с. Production preset не переключён: блокирующие доказательства — завершённые multilingual reindex/OCR, `index_readiness=true`, свежий authenticated ACL smoke с `acl_results_checked > 0` и сравнительные baseline/release_v2/reranker artifacts на том же source fingerprint.
