...
7. NO_GO 2026-07-11: retrieval eval и decision gate реализованы; controlled legacy baseline на 32 запросах дал recall@10 0.96875, MRR 1.0, p50 1.924 с, p95 3.998 с. Создана human review queue из 32 запросов с top-10 candidates, но без автозаполнения ground truth; finalize требует reviewer identity/time, минимум 3 no-answer и 3 forbidden cases. Текущий индекс остаётся production baseline. До GO data owner размечает queue, затем выполняются `release_v2`, multilingual dense и multilingual reranker в shadow collection; нулевой ACL leakage принимается только при `acl_results_checked > 0`.
