...
6. IMPLEMENTED 2026-07-11: единый admin UI/API health snapshot объединяет registry/telemetry DB, object storage, Qdrant, pending jobs, index coverage, backup freshness и restore drill; HTTP responses/logs/audit получают correlation ID. До DONE добавить отдельный worker heartbeat и queue-lag thresholds вместо одного pending count.
