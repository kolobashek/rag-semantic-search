...
- DONE 2026-07-11: machine-readable `pilot_gate` объединяет health/recovery/UI-audit/full-pytest/retrieval/sign-off, сверяет source fingerprints и не допускает ложный GO; текущий verdict после `595 passed` — `NO_GO` только по human-labeled retrieval/no-answer/ACL evidence и фактическому customer sign-off/update rehearsal.
...
7. NO_GO 2026-07-11: retrieval eval и decision gate реализованы; controlled legacy baseline на 32 запросах дал recall@10 0.96875, MRR 1.0, p50 1.924 с, p95 3.998 с. Создана human review queue из 32 запросов с top-10 candidates, но без автозаполнения ground truth; finalize требует reviewer identity/time, минимум 3 no-answer и 3 forbidden cases. Controlled `release_v2` сохранил recall/MRR, улучшил nDCG и p50, но p95 4.012 с не прошёл 3-секундный gate, поэтому production preset не переключён. До GO data owner размечает queue, после чего повторяются legacy/release_v2 и только затем multilingual dense/reranker shadow experiments; нулевой ACL leakage принимается только при `acl_results_checked > 0`.
