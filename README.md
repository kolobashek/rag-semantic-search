...
Интерактивный EXE устанавливается в профиль пользователя и позволяет выбрать расположение
облака, автозапуск, букву отдельного виртуального диска и режим хранения всего диска офлайн.
Виртуальный диск ссылается на тот же Files On-Demand root и не создаёт вторую копию данных.
После подключения эти настройки, отдельные офлайн-папки, текущая активность и ручная
синхронизация доступны через значок в системном трее. Проводник показывает нативные
CfAPI-статусы online-only, downloading,
locally available и always available; невыбранные файлы остаются видимыми и загружаются
при первом открытии. Клиент ограничивает реально выделенный локальный кэш и сохраняет
заданный резерв свободного места: давно неиспользуемые синхронизированные placeholders
обезвоживаются по LRU, а открытые, недавно использованные и закреплённые офлайн-папки
не очищаются. Порог кэша и резерв места задаются в установщике и настройках клиента.
Локальные создания, изменения, переименования и удаления передаются
на сервер; Windows завершает удаление placeholder только после подтверждения операции
сервером, а статус `Синхронизировано` выставляется после успешной операции. Токен device
login хранится с DPAPI текущего пользователя.

Клиент регистрирует в Windows 10 классическое подменю `RAG Cloud` для файлов и папок
в sync root: внутренний доступ, копирование ACL-защищённой ссылки и постоянное офлайн-хранение.
Для основного контекстного меню Windows 11 используется нативный `IExplorerCommand` в MSIX.
Пакет обязан быть подписан доверенным сертификатом организации; неподписанная сборка намеренно
не публикуется в манифесте автообновлений:

```powershell
packaging\build_cloud_files_shell.ps1 `
  -CertificatePath C:\secure\rag-cloud-files.pfx `
  -CertificatePassword $env:RAG_CLOUD_SIGN_PASSWORD `
  -Publisher "CN=TSK-NSK"
```

Клиент проверяет stable-манифест `/api/sync-client/version` при запуске и каждые шесть часов.
Новая версия скачивается с авторизацией, проверяется по размеру и SHA-256, после чего
устанавливается отдельным staged updater без сброса входа и пользовательских настроек.
Старый двусторонний sync client остаётся доступен отдельно до завершения тестов конфликтов записи.
