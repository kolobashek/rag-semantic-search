...
облака, автозапуск и режим хранения всего диска офлайн. После подключения эти настройки,
отдельные офлайн-папки, текущая активность и ручная синхронизация доступны через значок в
системном трее. Проводник показывает нативные CfAPI-статусы online-only, downloading,
...
на сервер; статус `Синхронизировано` выставляется только после подтверждения операции
сервером. Токен device login хранится с DPAPI текущего пользователя.
