...
class RAGSearcher:
    ...
    def search(
        ...
        results = self._merge_ranked_results(lexical_results, results, limit=limit)
    ...
    def _query_terms(self, query: str) -> List[str]:
        terms = [
            t.lower()
            for t in re.findall(r"[a-zа-я0-9\-]{2,}", query or "", flags=re.IGNORECASE)
        ]
    ...
    def _lexical_catalog_search(
        ...
        *,
        ...
        for item in self._refresh_fs_cache():
            ...
            elif not is_folder and ("псм" in hay or "птс" in hay):
